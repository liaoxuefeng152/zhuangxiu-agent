...
def test_coze_with_simple_image():
    """测试扣子API使用简单图片URL"""
    print("=== 测试扣子API使用简单图片URL ===")
...
- 直接返回JSON对象，不要用```json```包裹"""
    ...
    # 测试不同的图片URL
    test_cases = [
        ("简单公开图片URL", "https://img.alicdn.com/imgextra/i4/O1CN01Z5p5LZ1M1J5Q6X8q5_!!6000000001378-2-tps-800-600.png"),
        ("OSS签名URL（简化）", "http://zhuangxiu-images-photo.oss-cn-hangzhou.aliyuncs.com/quote/1/test.jpg"),
        ("无图片URL", None)
    ]
    ...
    for test_name, image_url in test_cases:
        print(f"\n--- 测试: {test_name} ---")
